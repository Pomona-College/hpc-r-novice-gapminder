...
eventually everything is a bit mixed together. On Sagehen, this often shows up as a `/bigdata/lab/<labname>/` directory containing six versions of the same CSV, three Word manuscripts named `final_v2_REAL.docx`, and Cecil Sagehen's slightly desperate post-it notes. RStudio Projects are a cure for this.
...
```{.output}
-rw-r--r-- 1 runner runner 82K Aug 24 22:22 data/gapminder_data.csv
```
