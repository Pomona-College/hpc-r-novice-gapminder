...
eventually everything is a bit mixed together. On Sagehen HPC, this often shows up as a `/bigdata/lab/<labname>/` directory containing six versions of the same CSV, three Word manuscripts named `final_v2_REAL.docx`, and Cecil Sagehen's slightly desperate post-it notes. RStudio Projects are a cure for this.
...
Download the [gapminder dataset (gapminder_data.csv)](data/gapminder_data.csv).
...
```{.output}
-rw-r--r-- 1 runner runner 82K Aug 27 15:38 data/gapminder_data.csv
```
