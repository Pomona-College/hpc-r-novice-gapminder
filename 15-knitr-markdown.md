...
the ![](fig/visual_mode_icon.png){alt='visual mode icon, the letter A with a compass rose'}
...
## Rendering reports without opening RStudio

Knit is a button in RStudio, but it is just a function call — which means a
report can be rebuilt by a job whenever its data changes:
...
Put that in an `sbatch` script and your report regenerates unattended. Combine
it with Make (Workshop 6) so the report only rebuilds when the underlying data
actually changed, and you have the beginnings of a reproducible pipeline —
which is Workshop 21.
